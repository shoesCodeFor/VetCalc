...
public class GenMedsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
         ...
         */
        final String [] medList = new String[meds.genMedications.length];
        final String [] doseRangeLbl = new String[6];
        Arrays.fill(doseRangeLbl, " ");
        doseRangeLbl[0] = "(Min-Max)";
        final double [] tempRange = new double[6];

        // Fill our Spinner with medications
        tools.fillMedList(meds.genMedications, medList);
        ...
        medSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
                ...
                tools.getDoseRangeLg(meds.genMedications, position, tempRange);

                // Build a new Dose object and initialize (name, concentration and range)
                final Dose oneDose = new Dose(
                        medList[position],
                        Double.parseDouble(meds.genMedications[position][1]),
                        tempRange
                );
